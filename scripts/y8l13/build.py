# -*- coding: utf-8 -*-
"""Slide generator for the Y8 L13 房子 (House) deck series.

Six decks, one per classroom lesson, from docs/lesson-plans/y8-l13/.
Same machinery as scripts/y9l12: one deck folder per lesson, a three-slide
vocabulary cycle, the classroom typography floor. Nothing here invents
Chinese — every sentence comes from the lesson plans, which come from
轻松学中文 2 pp.122–131 and the workbook pp.144–153.
...
Constraints this file enforces so they cannot drift:
  ...
  · a word with no picturable referent gets a textonly card, not a forced
    ...
    both exports run through Chromium
  ...
  · no pinyin-discrimination slides and no radical / simple-character
    ...
    Ex. 1 (z/zh, CD 62) and Ex. 7 (光 金 匕 入) and workbook Ex. 10 and 13
    are out of the series entirely. Stroke-order copying of the lesson's
    OWN new words (s_strokes, Lesson 5) is vocabulary work, not radical
    practice, and stays.

Run:
    python3 scripts/y8l13/art.py     # 27 drawings first
    python3 scripts/y8l13/build.py   # tokens.css + slides + deck shells
"""
import os, json, importlib

ROOT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..")
OUT = os.path.join(ROOT, "index", "designs", "y8-l13")
...
def s_strokes(rows, head, note=None):
    ...
    helper here at all."""
    tight = len(rows) > 4
    row_s = ' style="padding:6pt 16pt;"' if tight else ""
    hz_s = ' style="font-size:32pt;min-width:44pt;"' if tight else ""
    out = [label(head).rstrip("\n"),
           '  <div class="stack" style="gap:%dpt;">' % (5 if tight else 7)]
...
def s_list(head, rows, note=None, compact=False):
    # Eight or nine compact rows still overflow — there is a second notch,
    # and 14pt is 28px on the projector, still above the 26px floor.
    packed = compact and len(rows) > 7
    pad = ' style="padding:%dpt 14pt;"' % (5 if packed else 8) if compact else ""
    num = ' style="font-size:%dpt;min-width:19pt;"' % (17 if packed else 19) if compact else ""
    main = ' style="font-size:14pt;"' if packed else ""
    out = [label(head).rstrip("\n"),
           '  <div class="stack" style="gap:%dpt;">' % (4 if packed else 6) if compact
...
def build_deck(slug, title, slides, pptx=True):
    ...
    for n, s in enumerate(slides, 1):
        html = PAGE.format(title="Y8 L13 · " + s["label"], tag=s["tag"],
...
# ── Series landing page ────────────────────────────────────────────
# index/designs/y8-l13/index.html — the page the gallery card links to,
# and the page thumb.png is a screenshot of. Generated like everything
# else under index/designs/y8-l13/; nothing there is hand-maintained.
INDEX = """<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Y8 L13 · 房子 House</title>
...
<div class="wrap">
  <p class="kicker">轻松学中文 2 · Unit 5 · Lesson 13</p>
  <h1>房子</h1>
  <p class="sub">House · Year 8</p>
  <p class="meta">Six 50-minute lessons, {slides} slides. Nineteen new words spread across the sequence — two to six per lesson, each pair taught as a three-slide cycle: the words, then the words used, then the students using them. Every lesson carries a CFU board straight after the new pattern, question-only: there is no answer slide anywhere in the series. Revision happens inside every lesson rather than in a review lesson at the end.</p>
  <div class="rule"></div>
  <div class="focus">
    <h2>Grammar Focus · 本课语法点</h2>
    <div class="fgrid">
      <div class="fcell"><p class="l">L1</p><p class="p">有　／　在</p><p class="g">地方 + 有 · 东西 + 在 — what a place contains, and where a thing is</p></div>
      <div class="fcell"><p class="l">L2</p><p class="p">几 + 间 + 卧室</p><p class="g">间 for bedrooms, 个 for everything else</p></div>
      <div class="fcell"><p class="l">L3</p><p class="p">在 + 地方 + 做什么</p><p class="g">Saying what someone is doing where</p></div>
      <div class="fcell"><p class="l">L4</p><p class="p">在 + 地方 + 面</p><p class="g">上 / 下 / 里 / 外 / 左 / 右 — and the same fact reversed</p></div>
      <div class="fcell"><p class="l">L5</p><p class="p">住什么样的房子？</p><p class="g">House type, and 房前 ／ 房后</p></div>
      <div class="fcell"><p class="l">L6</p><p class="p">间 · 个 · 层 · 辆</p><p class="g">All four measure words, which the unit test asks for directly</p></div>
    </div>
  </div>
  <div class="grid">
{cards}
