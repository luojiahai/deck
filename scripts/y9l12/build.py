# -*- coding: utf-8 -*-
"""Slide generator for the Y9 L12 外出就餐 (Eating Out) deck series.

Six decks, one per classroom lesson, from docs/lesson-plans/y9-l12/.
Same system as y9-l10 菜市场 and y9-l11 零食: same chrome, same three-slide
vocabulary cycle, same typography floor. Nothing here invents Chinese — every
sentence comes from the lesson plans, which come from 轻松学中文 3
pp.110–121 and the workbook pp.128–137.
...
Constraints this file enforces so they cannot drift:
  ...
  · a word with no picturable referent gets a textonly card, not a
    forced drawing
  · CFU slides carry questions only — s_cfu has no answer parameter, so an
    answer slide cannot be added by accident (the teacher asked for none)
  · no pinyin-discrimination and no radical/component slides in this series:
    s_components is deliberately absent

Run:  python3 scripts/y9l12/build.py
"""
import os, json, importlib

ROOT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..")
OUT = os.path.join(ROOT, "index", "designs", "y9-l12")
...
def build_deck(slug, title, slides, pptx=True):
    ...
    for n, s in enumerate(slides, 1):
        html = PAGE.format(title="Y9 L12 · " + s["label"], tag=s["tag"],
...
def main():
    ...
    for mod_name in LESSONS:
        mod = importlib.import_module("scripts.y9l12." + mod_name) if __package__ else \
